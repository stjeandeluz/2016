...
> # generate a Monte Carlo sample and compare toPrProb <-  the Z scores
error <- replicate(B,{
...
```{r}
> # Doing a sample model for a single year of loans, modelling a default as 1, with 1/50 probability, else no default
> # sum up the '1' values and multiply by $120k to get $ losses for the year
 
> loans <- 1000

> outcome <- replicate (1, {
+ L <- sample (c(0,1), loans, replace=TRUE, prob=(c(49/50,1/50))  )
+ sum(L)
+ }  )
> # The above single run of code showed 21 losses, so the bank would lose 21 * $120,000  or $2.52 million from default

> # This result is fairly close to the expected 20 defaults. More replications should show an even closer average:
