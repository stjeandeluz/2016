...
```{r}
> # As noted, the loss probability is a bit higher. Using the hints, will use the MC model of Problem 2E and toggle the probabilities/Z score.
> # Chance of default = 5% with same interest rate:
> 
> profit <- replicate(10^4,{
+                                            P <- sample( c(-120000,4176), 1000, replace=TRUE, prob=(c(1/20,19/20)) )
+                                             sum(P)
+                                         })
> 
> mean(profit < 0)
...
> # Chance of default = 1% with same interest rate:
> profit <- replicate(10^4,{
+                                                      P <- sample( c(-120000,4176), 1000, replace=TRUE, prob=(c(1/100,99/100)) )
+                                                       sum(P)
+                                                   })
> mean (profit < 0)
[1] 0
...
> # Varying the interest rate now.
> # interest rate = 5% with original probabilities:
> # outcome for non-defaults = .05 * 180k = 9k
> profit <- replicate(10^4,{
+                                                P <- sample( c(-120000,9000), 1000, replace=TRUE, prob=(c(1/50,49/50)) )
+                                                  sum(P)
+                                              })
> mean(profit < 0)
[1] 0
> # interest rate = 1% with original probabilities:
> # outcome for non-defaults = .01 * 180k = 1800
>  profit <- replicate(10^4,{
+                                                          P <- sample( c(-120000,1800), 1000, replace=TRUE, prob=(c(1/50,49/50)) )
+                                                            sum(P)
+                                                       })
> 
> mean(profit < 0)
...
When I ran the code above, I was first varying the interest rate above and below the optimum 2.32%. At 5% there was virtually 0% chance of a loss via MC, and plugging into the -EP/SE formula (not shown here; see 2E) yielded Z score > 3.5, with a small but positive probability of loss (so tiny difference between Z and MC probabilities). Doing similar for a rate of 1% yielded a Z score of 1.17, with loss probability 87.9%, with a greater difference ( the mean MC chance of loss was 89.8% ). So as interest rate increases, the difference is smaller. Because we set it to minimize loss by an exacting criterion, setting it even higher will make positive profit even more assured, under MC or CLT. 

Looking at it from the same interest rate but changing the probability of default, when the probability is only 1%, The Z score > 4 so there is a small but positive difference in the Z and the MC result of 0. When the probability increases to 5%, there is a Z score of 2.38 with associated loss probability of .9913 vs. .9927 in the Monte Carlo run. Like before, when the interest rate is set "hard" to minimize long-run losses at Pr=.001 , making losses even more unlikely (less default), will make it easier for both methods to agree.

The discrepancy seems to come when you ease up on the criteria to "ensure" positive profit (higher default and/or lower interest rate). There is a greater range of outcomes that can realistically occur. CLT/Z score gives their probabilities based on infinite trials, whereas MC has a fixed number of draws, now with more possible outcomes, which would increase the standard error in relation to what can be approximated with the Z tables for those outcomes. It seems that the decreased precision shows up in the different probabilities under the 2 methods. 
...
> # To simplify, since the expected profit from before was very near $1.7MM, I will assume it exactly that amount for the calculations
> # As before, bank makes expected profit EP = 1000 *  [(1-p)*180k*int   +   p*(-120k)]
> # here p is .05 or 1/20
> # set EP = $1.7MM and solve
> amount1 <- (19/20)*180000*1000
> amount2 <- (1/20)*(-120000)*1000
> # so amount1*int + amount2 = $1.7MM   so   int = [$1.7MM - amount2] / amount1
> ans <- 1700000 - amount2
> ans <- ans / amount1
> ans
[1] 0.04502924
> # so need to make the interest rate about 4.5%
> # note this assumes they are also making 1000 loans each year
```
...
> # Using CLT, want to know what is the Z value associated with 0 profits, given that EP = $1.7MM, then see probability area below the Z value
> # TOP of Z value = 0 - EP = -$1.7MM
> # BOTTOM of Z value is the SE
> # from problems 2E and 3A, SE = sqrt(1000)*abs(180k*.045 + 120k)*sqrt(1/20*19/20)
> ans <- sqrt(1000*1/20*19/20)
> ans <- ans*((180000*.045) + 120000)
> SE <- ans
> Z <- -1700000 / SE
> Z
[1] -1.925542
> pnorm(Z)
[1] 0.0270808
> # so there is about a 3% chance of negative profits

> # Using Monte Carlo method:
> # noting expected profit = .045*180k  = 8100 for each loan that doesnt go bad
> MC <- replicate(10^4,{
+               X <- sample( c(-120000,8100), 1000, replace=TRUE, prob=(c(1/20,19/20)) )
+               sum(X)
+           })
> 
> 
> mean (MC < 0)
[1] 0.0305
# This says there is about 3% chance that profits will be negative; equivalent to Z-score result.
...
This is like problem 2E but now N is what we need to know. We have interest rate of 4.5% and a probability of default of 5%. From Z tables, get Z-score such that .001 of the area is below it (that is a Z-score of -3.09) and that should be equivalent to the point of zero profits. In other words, using CLT, what N will get that Z-score given the properties of the loan market?

As before, we can compute -EP/SE  =  -3.09     or   EP/SE  = 3.09 and solve for N. 

Below code does that and then confirms via Monte Carlo. 

```{r}
> # As before, want [0 -EP] / SE  = - 3.09   or  EP/SE = 3.09   What N can achieve this?

> # expected profit EP = N *  [(1-p)*180k*int   +   p*(-120k)]
> # or: N * ((19/20*180000*.045) + (1/20*-120000))
> a1 <- ((19/20*180000*.045) + (1/20*-120000))
> # so a1*N = expected profit
> # SE = sqrt(N)*sqrt(1/20*19/20)*abs((180000*.045) + 120000)
> a2 <- sqrt(1/20*19/20)*((180000*.045) + 120000)
> # so now (a1*N) / (a2 * sqrt(N))   =   3.09
> # same as (a1/a2)*sqrt(N) = 3.09   or  sqrt(N) = 3.09*(a2/a1) or N = (a2/a1)^2 * (3.09)^2
...
> N
[1] 2590.415
> # So this would require making about 2,600 loans that year.

> # Check this with Monte Carlo:
 
> MC <- replicate(10^4,{
+                    X <- sample( c(-120000,8100), 2590, replace=TRUE, prob=(c(1/20,19/20)) )
+                    sum(X)
+                })
> 
> mean(MC < 0)
[1] 0.0023
> # which still has the same value as before when comparing to the Z probability. Close, but some imprecision (see Question 2F).
```
...
```{r}
> # adding random variation to the process
> # interest rate is 4.5% on 2590 loans as in previous section.

> # Process 1 has independent outcomes:

> outcomes1 <- replicate(10^4,{
+              S1 <-      sample( c(-120000, 8100), 2590, replace=TRUE, prob=c(.05, .95))
+                sum(S1)
+             })
> mean(outcomes1 < 0)
[1] 0.0018
> # which matches to the .001 probability from before
> # and virtually no chance of losing $10MM and probably not even $1MM
> mean (outcomes1 < -1000000)
[1] 2e-04
> mean(outcomes1 < -10000000)
[1] 0

> # Process 2 has outcomes that are not independent; borrowers can either default at a 1% or 10% rate now

> outcomes2 <- replicate(10^4,{
+     add <- sample( c(-0.04,0.05), 1)
+   S2 <-  sample( c(-120000, 8100 ), 2590, replace=TRUE, prob=c(p2+add, 1-(p2+add)))
+      sum(S2)
+      })
> mean(outcomes2 < 0)
[1] 0.4988
> # now there is about 50% probability of loss
> # and losses tend to be large (this also stretches out the gains ... but whole business becomes more volatile)
> mean(outcomes2 < -1000000)
[1] 0.4988
> mean(outcomes2 < -10000000)
[1] 0.4355
...
There were a lot of complex reasons for the financial crisis, but the homework examples illustrate some basic points. The main variables we looked at were interest rate, default rate and number of loans. These factors used together could give an idea of expected bank profits and more generally, the likely values they could take. Banks could note the environment and offer the optimal number of loans at the optimal rate, anticipating a level of default.

In the easy money conditions before the crisis, interest rates became very low and banks were limited in how high a rate they could charge given the competition. Low interest rates do less to cover potential losses, yet during this period, there was also an offsetting increase in the number of loans (loans and interest rates tend to negatively correlate). But the default rate also increased sharply as the housing market crashed. The complex combination of low rates/easy credit, a large loan balance sheet for the banks and a climbing rate of default all made the banks vulnerable to large losses.

It is sort of like the roulette example, where a customer bets on the housing market. The casino(bank) offers a low price to bet so the customer plays, and they have optimized the odds to win in the long run under conditions that are stable. More and more customers come to play because it seems a cheap way to make money, but some of them are not able to pay out their losses and walk away from the table. So the casino's losses are substantial because they did not anticipate the different players who would visit and whether they were financially able. 
